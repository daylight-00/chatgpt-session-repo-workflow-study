...
The service is capable enough for real repository-oriented project work when the project is structured around its actual primitives. The main risk is not lack of capability but confusing the three state domains:
...
Once these are treated as separate systems with explicit handoff boundaries, the workflow is coherent and usable.
